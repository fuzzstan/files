...
func NewEndpoints(options map[string]interface{}) svc.Endpoints {
	...
	// Endpoint domain.
	var (
		getFilesEndpoint   = svc.MakeGetFilesEndpoint(service)
		getFileEndpoint    = svc.MakeGetFileEndpoint(service)
		createFileEndpoint = svc.MakeCreateFileEndpoint(service)
		updateFileEndpoint = svc.MakeUpdateFileEndpoint(service)
		deleteFileEndpoint = svc.MakeDeleteFileEndpoint(service)
	)
	...
	endpoints := svc.Endpoints{
		...
		UpdateFileEndpoint: updateFileEndpoint,
...
func RegisterService(cfg nserver.Config, r *mux.Router, s *grpc.Server) svc.Endpoints {
	...
	pb.RegisterFilesServer(s, svc.MakeGRPCServer(endpoints, tracer, logger))
...
func Run(cfg nserver.Config) {
	...
	// Run!
	logs.Info("files.FilesServer", " started.")
	logs.Info("files.FilesServer", <-errc)

	logs.Info("files.FilesServer", " closed.")
}
