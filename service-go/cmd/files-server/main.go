...
import (
	...
	// This Service
	"github.com/fuzzstan/files/service-go/internal/files-server"
)
...
func main() {
	...
	err := config.ScanFrom(&conf, "filesServer", "server")
	if err != nil {
	...
	if len(gitHash) == 0 {
		logs.Infow("starting FilesServer, you can use `-ldflags \"-X main.gitHash=xxx\" to generate gitHash")
	} else {
		logs.Infow("starting FilesServer", "gitHash", gitHash, "gitBranch", gitBranch, "buildTime", buildTime)
	}
